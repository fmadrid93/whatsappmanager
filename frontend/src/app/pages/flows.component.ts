    ...
    <main class="page flow-page">
      ...
      <p-card header="Flujos registrados" styleClass="flow-table">
        <p-table [value]="flows()" [tableStyle]="{ 'min-width': '980px' }">
          ...
          <tr>
            ...
            <td>
              <div class="flow-actions">
                ...
                @if (flow.isTemplate) {
                  <p-button label="Clonar" icon="pi pi-copy" size="small" severity="help" (onClick)="startClone(flow)" />
          ...
          @if (cloningFlow()?.id === flow.id) {
            <tr class="clone-row">
              <td colspan="7">
                <div class="clone-form">
                  <div class="clone-form-header"><i class="pi pi-copy"></i> Crear un bot nuevo a partir de "{{ flow.name }}"</div>
                  <label for="clone-name">Nombre del nuevo bot</label>
                  <input pInputText id="clone-name" name="cloneName" [(ngModel)]="cloneName" [ngModelOptions]="{ standalone: true }" />
